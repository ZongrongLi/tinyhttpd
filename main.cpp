...
void *accept_request(void* data)
{
    ...
    if(strcmp(url,"/?")==0 ){
        ...
        while ((numchars > 0) && strcmp("\n", buf))  /* read & discard headers */
          ...
          sprintf(buf, "\r\n");
          send(client, buf, strlen(buf), 0);
          sprintf(buf, "<HTML><TITLE>helloworld</TITLE>\r\n");
          send(client, buf, strlen(buf), 0);
          sprintf(buf, "<BODY><P>body P\r\n");
          send(client, buf, strlen(buf), 0);
          sprintf(buf, "line.\r\n");
          send(client, buf, strlen(buf), 0);
    ...
    printf("path:%s url:%s query_string:\n",path,url,query_string);
    /*根据路径找到对应文件 */
    if (stat(path, &st) == -1) {
        printf("1\n");
        /*把所有 headers 的信息都丢弃*/
...
printf("%s-%s\n",buf,path);
    ...
    if (pid == 0)  /* child: CGI script */
    {
        ...
        //execl("htdocs/test","htdocs/test",NULL);
        exit(0);
    } else {    /* parent */
...
int main(void)
{
    ...
    setsockopt(server_sock,SOL_SOCKET,SO_REUSEADDR|SO_REUSEPORT,&opt,1);
